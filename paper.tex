...
processing tasks on a single set of data. In addition to carrying out such
...
such work are \emph{parameter sweeps} where tasks are run over a series of
...
conveniently.  These tasks include visually formulating a task, composing
and marshalling of workflow, browsing and selecting data, visualizing and
sharing results. 
...
SPIs~\cite{maheshwari-missier-etal:2009}. Recent implementations of Taverna
offers a client-server based environment in which a Taverna server can remotely
serve the workflow composition and execution services. This mode is useful in
the cloud computing environments where the server decides where to run the
Taverna services in the cloud.
...
uses the Modeling Markup Language (MoML) for its
