...
help checkpoint methods and reproduce and modify partial computation. Automated
...
%
\paragraph{Pegasus/DAX} Pegasus \cite{deelman-peg} is an SWE that is well-suited
to generate and execute large abstract DAGs made up of tasks with dataflow.
Abstract description is in the form of a DAG via a language called DAX. The
concrete expression takes into account the properties of application task
requirements and underlying compute resources. A Pegasus workflow is expressed
in a simple XML-based language called DAX (DAG in XML). DAX describes the
logical workflow components, their inputs and outputs and their links and
dependencies in a Pegasus workflow. These kind of workflows has been shown to
enact optimally using Pegasus' smart memory and disk management techniques.
...
clusters, Grids and supercomputers. Pegasus manager buffers and maintains a
fix-sized window of executable tasks before accepting new tasks.  Regarding
massively parallel task scheduling, Pegasus provides a task
clustering technique that groups parallel jobs based on fixed parameters such
as number of groups. Recent developments in Pegasus enables it to run workflows
on cloud environments~\cite{peg-cloud}.
...
workflow specification in the form of Pegasus/DAX workflows and generates
concrete executable workflows. It generates workflows by selecting components,
data sets, and parameters in three stages while eliminating candidates that are
not viable.

% How does Wings achieve the translation between abstract and concrete workflows?
% What are the internal representations of workflows?
...
Business Process Execution Language (BPEL) is a workflow
...
\cite{bpel}. However, the language does not suit well to the
