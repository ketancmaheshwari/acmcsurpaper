...
to the large-scale experiments, for which scientific users turn to
automation, their demands for computational power often exceeds that of a
single personal computer. Such applications are often composed of multiple,
distinct computational stages making them well-suited to the workflow
paradigm. These properties also make them ideal candidates for a
large-scale distributed computing environment. The case for suitability of
SWEs on large-scale distributed infrastructures has been well-argued upon
in~\cite{jha-katz-etal:2009}. In such a scenario, taking explicit steps to
ensure scalable and optimized performance is vital.
%

\item \textbf{Data Management.} The scale of data transfer is much higher in
SWEs from one execution site to others as well as among storage resources
while it is lower (ranging from variables to small data structures) in
programs.
%

\item \textbf{Interface to Distributed Computing Infrastructures (DCIs).}
Building SWE's interfaces to distributed systems is a complex task. The
...
correctly. Further complexity is added because of complex dataflow
requirements, interactions with diverse tools and linking with legacy
application codes.
...
determined by the data availability for execution. This model of
computation is known as dataflow computing. A workflow thus does not follow
a fixed order of execution and different execution sequences are possible
which are dominated by dataflow. The unit of work in SWEs is at a higher
level of granularity. A unit of execution is coarse-grained, and often
termed as ``processor'', ``task'', ``service'' or ``actor'' in the SWE
community. This notion of high-level granularity drives the depth to which
a workflow can be expressed in a given language form. In the rest of this
paper, this unit of work will be referred to as \textit{processor}. On the
other hand, a unit of execution in most programming languages is a much
simpler and finer executable statement that accomplishes a small
computational task. This presents a whole new set of requirements for
expressing the processors and how they should interact and operate upon
data.
...
composition environment albeit with visually assisting programming
environments such as IDEs. However, they are seldom to the extent that a
non-expert user can graphically compose programs.
...
Figure \ref{fig:wfms} shows a high-level architecture of a SWE. It shows key
...
\textit{workflow engine} using commands and messages. The \textit{workflow
language} becomes an intermediate representation for communication between user
interface and the \textit{workflow engine}. The workflow engine acts as a
compiler/interpreter and scheduler for the workflow tasks and interfaces with
the underlying \textit{distributed resources}. It also monitors the workflow
progress and collects the intermediate and final results.
...
MOTEUR has sophisticated data management mechanisms within the workflow
definition. It provides a diverse set of iterations strategies well adapted for
different combinations of input datasets at the processor. An iteration
strategy allows different combinations of the data arriving at a processor
along its multiple inputs. Running legacy application code on grids has been
eased by MOTEUR by providing a generic grid-service wrapper for these
applications \cite{glatard-montagnat-etal:2008a}. Grid-services thus created
can be readily deployed on the grids by the MOTEUR workflow enactor. MOTEUR
workflows are written in the scufl language and are compatible with those
written for the Taverna SWE. It also supports an extension of scufl language
called the Gwendia language, developed at the same laboratory. 
%
...
specification for webservices \cite{taylor-wand-etal:2005}. A highlight of
...
metaphor where an actor represents a unit of execution and the director
...
%
\paragraph{Dryad/--} The GridNexus \cite{GridNexus} SWE, developed at the
...
underlying resources using intelligent and dynamic scheduling policies. A
...
of executable tasks before accepting new tasks.

%

\paragraph{Swift/Swift Script} Swift \cite{swift} is a script-based scientific
workflow environment developed at the University of Chicago. A Swift workflow
specified in the SwiftScript scripting language is translated to an
intermediate XML-based representation and enacted by the Karajan workflow
enactment engine. The SwiftScript language is proposed as a workflow language
founded on a scripting approach programmers are familiar with. To make the
language adapted to grids, the inventors of SwiftScript generalized the use of
futures \cite{Halstead85multilisp:a} for every variables defined in the script.
Futures are non-blocking assignment variables, using a proxy to ensure
...
data enables blocked thread to restart execution. The beauty of futures is to
make this process completely hidden from the programmer who can use a scripting
language he is familiar with without ever manipulating explicit parallel
programming constructs. Swift facilitates a highly asynchronous workflow
execution. Swift supports advanced workflow constructs such as parallel loops
and conditionals. Furthermore Swift supports processing of explicitly defined
arrays of data. Swift workflows take advantage of POSIX-style shared file
system to reuse the data produced by other processors.
...
\cite{fahringer-prodan-etal:2007} is developed at the University of Innsbruck.
...
\paragraph{The P-GRADE Portal/--} The P-GRADE Portal \cite{kacsuk-sipos:2005}
...
%
\paragraph{DaGue/--} Bioflow \cite{bioflow} is a query based workflow language.
...
by Galaxy using their own Galaxy server or a web-server exposing the webservice
descriptions through public WSDLs. One of the major use of Galaxy is to achieve
a mix and match of data analysis tools in order to analyze a vast repository of
...
workflow and pipeline. The optimization features considered are the presence of
