...
bulk data processing or running multiple distinct processing on a single set of
data. In addition to carrying out such work, workflow environments provide
oppurtunities for giving structure and order to a set of tasks and often with
sufficient expressivity, choices and selections to perform tasks selectively.
...
browsing and selecting data, visualizing results. Furthermore,
dissemination of methods and results to communities and collaborators are
some of the activities that SWEs make convenient for end-users.
...
% How SWEs differ from traditional programming languages and runtimes
...
%TODO: Add content about new and emerging Distributed Computing Infrastructures: Clouds, GPUs and exascale supercomputers
...
The present work analyzes contemporary SWEs in order to find the capabilities
and key features in terms of the characteristics described in the introduction
part of this paper. The rest of this paper is organized as follows: Section
...
this work since we rely on the peer reviewed publications for information on a
given SWE. Some features, such as extendability of SWE strongly depend on the
extension points designed in the code; consequently, we consider it present in
only those SWEs that have a documented description or demonstration of the
given feature. 
...
1. Swift has been shown to run on multiple clouds~\cite{ccgrid2013} via its
coasters mechanism~\cite{ucc2011}.

2. Swift/T~\cite{turbine2013}, runs scientific workflows over
HPC resources. Have been shown to work on petascale supercomputers such as the
IBM BlueGene and Cray series of machines.
...
%developed as a part of the current work is similar in spirit to the
%SwiftScript. Similar to SwiftScript, gscript adopts futures to express
%asynchronous execution in a script-based language. A SwiftScript workflow
%contains statically typed datasets that can be specified using ``annotations''
%in the script. Unlike SwiftScript, gscript does provide user-defined
%strategies in order to effect data combinations in the workflow. SwiftScript
%provides for fault-tolerance and exception handling at run-time which is
%lacking currently in gscript. The underlying datasets organization in swift is
%represented using an XDTM XML-representation. This means that effectively, a
%workflow is SwiftScript plus the XDTM data representation. A workflow is
%simpler and compact in gscript as its data description is embedded within a
%workflow representation. SwiftScript supports creation of nested-workflows in
%the form of compound procedures. Conditional and Loop execution is supported
%using the if and for/while statements in both the languages.
